...
- **온톨로지 예외**: 이 문서는 승인된 합의 계획(ralplan pending-approval, 스펙 fact-1/8)이 `docs/research/`에 지정한 결정-지향 산출물이다. 통상의 research 계약(pre-decision 사실 수집)에 대한 **명시적 예외**이며, 여기 기록된 처분·배치 범위는 합의 계획의 승인 권한(IR-5) 아래 확정된 결정이다. 후속 작업의 정본은 본 문서 + audit-matrix.md다.
...
25개 omo 스킬 전수. 검증: `python3 scripts/governance/tools/count_ledger_rows.py docs/research/omo-analysis.md --section "처분 원장" --expect 25 --ledger --names-file scripts/governance/fixtures/omo-skills-25.txt` (행 수 + 헤더/셀/중복/처분값 + 고정 25-이름 집합 동치)
...
- **backend**: FastAPI/uv·Express/zod를 greenfield 기본값으로만(:65-79); folder triad는 후보 신호(:16-31); rule-of-three를 소유권/결합도 판단으로; persistence의 greenfield Prisma 기본도 동일 원칙(references/persistence.md:26-43).
...
- **write-prd** (신규): 518줄 template에서 안내문/예시 제거(artifact frame만 유지 — v1.0.1 CHANGELOG 주장과 현 template 불일치 해소, templates/prd.template.md:16-37,447-518); checklist를 실제 섹션과 일치화; 목표 ≤3·고정 fill order를 리뷰 신호로 완화.
