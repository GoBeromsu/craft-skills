...
| **GJC** (Gajae-Code) | Marketplace plugin: `gjc plugin marketplace add GoBeromsu/craft-skills` then `gjc plugin install craft-skills@craft-skills`. GJC loads packages straight out of the installed plugin and advertises them as `craft-skills:<name>`; `gjc plugin upgrade` is the whole update path. Never copy a package into `~/.gjc/agent/skills/` and never point `skills.customDirectories` at the versioned plugin cache — both pin a stale copy that an upgrade cannot replace. |
