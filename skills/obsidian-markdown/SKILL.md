...
description: Creates, edits, and reformats Markdown notes in Obsidian with wikilinks, embeds, callouts, properties, tags, math, Mermaid fences, and a headings-first house style. Use for meeting notes, journals, summaries, captured ideas, or cleanup that needs short one-idea bullets, tables for comparisons, numbered sequences, body-date and person wikilinks, and inline web links. Not for `.base` data views (use obsidian-bases) or `.canvas` graphs (use obsidian-canvas).
metadata:
  version: 4.0.0
---
