...
description: Create importable JSON templates for the Obsidian Web Clipper — properties, note-content format, triggers, and note-name/path rules — with every selector verified against a real page before it ships. Use when building a new clipper template for a site (YouTube, GitHub) or content type (Recipe, Article), mapping clipped fields to vault frontmatter, or debugging clipper variables and filters. Not for extracting page content on the command line (use defuddle) or authoring the note body format (use obsidian-markdown).
metadata:
  version: 1.0.0
---
...
4. **Fetch & Analyze Reference URL:** Validate variables against a real page.
    ...
    - **(REQUIRED)** Retrieve the page content with a web fetch/extraction step (e.g. `defuddle`). See [references/analysis-workflow.md](analysis-workflow.md).
