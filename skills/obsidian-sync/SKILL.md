---
name: obsidian-sync
description: Operates the Obsidian headless Sync client (`ob`) safely across a primary vault and headless replicas. Use when someone says “headless sync 점검”, “볼트 동기화 복구”, “pull-only로 맞춰줘”, “sync daemon restart”, “obsidian sync status”, or asks to bootstrap, reconfigure, inspect, or recover an `ob sync --continuous` replica. Stages every first sync in pull-only mode, verifies the resulting diff before promotion, and requires a rollback point before any mutating operation. Not for the Obsidian desktop Sync settings UI, general vault note editing, or non-Obsidian file replication.
metadata:
  version: 4.0.0
---
...
Use `obsidian-cli` for note reads and writes, `obsidian-doctor` for plugins and templates, and the desktop app for GUI Sync settings.
...
Follow [recovery.md](references/recovery.md); do not “try bidirectional” to see whether it heals itself.
...
Use [daemon-operations.md](references/daemon-operations.md) for supervisor configuration and remote inspection.
...
See [cli-commands.md](references/cli-commands.md) for the command surface and [recovery.md](references/recovery.md) for incident containment and restoration.
