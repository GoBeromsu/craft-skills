...
2. Remove only the stale local pairing.
3. Run `sync-setup` in a real TTY.
4. Set `pull-only` before the first sync.
...
- Record the affected machines and vault paths.
- Record the first observed symptom and earliest relevant error.
- Record process and client state at containment.
- Record the rollback source and exact revision or backup timestamp.
- Record file-count and diff evidence before and after restoration.
- Record the one-shot pull-only and bidirectional verification results.
- Record the final supervisor state.
