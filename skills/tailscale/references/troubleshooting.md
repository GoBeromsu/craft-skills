...
- **Wrong restart path**: `brew services restart tailscale` only restarts a per-user LaunchAgent. On a host running the system LaunchDaemon (`/Library/LaunchDaemons/com.tailscale.tailscaled.plist`), run `launchctl kickstart -k system/com.tailscale.tailscaled` as root instead. The two are not interchangeable.
