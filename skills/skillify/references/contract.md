...
- Structure: title → the contract block (`## Goal`, `## Output contract`) → the workflow/decision content → `## Non-goals` → `## Failure modes` → any `## Requirements`, `## Anti-patterns`, `## Verification`. Cut preamble and restated-obvious practice — an agent is already competent; only add context it doesn't already have.
- The contract block is what the evals grade against, so every package carries these four sections as literal `##` headings (the validator checks for them):
  - `## Goal` — 1–3 sentences: the one observable job, the user outcome, and a success condition a judge can check. Name the inputs the skill needs when they are not obvious from the trigger.
  - `## Output contract` — exactly what a correct run leaves behind: artifact and location, format and required sections, links or field rules, the summary returned to the user, and the no-result behavior. Specify only constraints that matter; where several surface forms are equally valid, say so instead of pinning one.
  - `## Non-goals` — explicit exclusions, the sibling that owns each excluded case, destructive actions the skill never takes, and when it escalates instead of acting. The description's "Not for X" sentence is the compressed form of this section.
  - `## Failure modes` — missing or invalid inputs, an unavailable tool or network, ambiguity, and partial success: for each, the safe stop or recovery and the error the user sees.
  - The trigger lives in the description (§3); the inputs and dependencies live in `## Goal` and `## Requirements`; the eval corpus that proves the contract lives in `tests/evals/` (§7).
