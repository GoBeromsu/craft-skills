...
| `CHANGELOG.md` | Always, alongside any versioned SKILL.md. Append one **compact line** per change — `- YYYY-MM-DD — [vX.Y.Z: ]<why it changed> → <what it became>.` — leading with the trigger and linking any referenced skill/file/source with `[text](path)`. One line, summary not diff; full detail lives in git. Newest last; never edit prior bullets (a one-time reformat to this convention is the lone exception). |
