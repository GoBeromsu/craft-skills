...
Success looks like: the package passes both Layer-1 validators, its `## Output contract` and committed eval corpus agree, its description triggers correctly and only on the intended prompts, and its [delivery follows the lifecycle flow](references/lifecycle.md#6-branch--commit--pr).
...
- The with-skill arm shows no delta over the baseline → do not ship the body; sharpen the recipe or record that the capability is already native.
...
- Dirty tree or stale `main` → use the [clean-start route](references/lifecycle.md#1-clean-start) before editing; the lifecycle ends at an open PR on a clean branch, never at a merge or an edit to the user's checked-out branch.
