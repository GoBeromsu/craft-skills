...
and its [delivery follows the lifecycle flow](references/lifecycle.md#6-branch--commit--pr).
...
Choose the package operation from [`references/lifecycle.md`](references/lifecycle.md):
create (§2), update and record corrections (§3), move/rename (§4), or retire (§5). Before
editing, use its [clean-start route](references/lifecycle.md#1-clean-start) so unrelated
work stays untouched.

## Validate (Layer 1 — deterministic, CI-enforced)

Run the [validator playbook](references/runtime-hygiene.md#2-validator-playbook) for each
changed package.
