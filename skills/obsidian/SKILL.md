...
- The official Obsidian.app `obsidian` binary for vault-aware note operations, documented at [Obsidian CLI](https://help.obsidian.md/cli). This is what "Obsidian CLI" means in this package.
- `${OBSIDIAN_CLI_PATH}` or a third-party `obsidian-cli` binary on `PATH` only as the headless fallback when the desktop app cannot run; it is a different project with a smaller surface, not the official CLI.
...
- The `init` skill's tool-preflight reference (`tool-preflight.md` under its references) distinguishes the Yakitrak `obsidian-cli` from the separate Obsidian.app `obsidian` binary and records their safe probes and CHANGELOG verification receipt convention.
...
| Read, create, search, move, mutate, or link-audit vault notes through the official `obsidian` CLI (or its headless `obsidian-cli` fallback) | [`references/cli.md`](references/cli.md) |
...
| Read Sync state for a vault the desktop app already syncs (`sync:status`, `sync:history`, `diff filter=sync`) | [`references/cli.md`](references/cli.md) |
...
4. **Use the least destructive surface.** Prefer the official `obsidian` CLI for vault-aware note operations. Treat delete, unlink, reset, mirror, cleanup, and bulk replacement as destructive operations requiring explicit scope and approval.
...
- Editing a vault note with raw shell text tools when the official `obsidian` CLI is available → use the vault-aware surface and exact readback.
- Treating the third-party `obsidian-cli` as the Obsidian CLI, or assuming one binary's commands exist on the other → route through `cli.md`, which names the official surface and scopes the fallback.
