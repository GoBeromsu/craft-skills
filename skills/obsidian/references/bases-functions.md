...
Functions are used in [[Introduction to Bases|Bases]] to manipulate data from [[PROPERTIES]] in [[views#Filters|filters]] and [[formulas]].
See the [[Bases syntax|bases syntax]] reference to learn more about how you can use functions.
...
Functions you can use with any value.
This includes strings (e.g. `"hello"`), numbers (e.g. `42`), lists (e.g. `[1,2,3]`), objects, and more.
...
Date comparisons can be done using [[Bases syntax#Date arithmetic|date arithmetic]].
...
Functions you can use on a link.
Links can be created from a file (`file.asLink()`) or a path (`link("path")`).
...
Functions you can use with a regular expression pattern.
Example: `/abc/`.
