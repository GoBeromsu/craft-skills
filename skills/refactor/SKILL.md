...
Restructures code so it is easier to work with without changing what it runs — the moment behavior changes, the work is a feature or a fix, not a refactor. Done well: tests stay green at every step, one mechanical move happens at a time, and the diff never mixes structure with behavior.
...
3. Run the suite after every step; red reverts that one step immediately rather than debugging forward on a refactor that just broke something.
...
- A commit/PR touches at most ~10-15 files before it needs a split into a stack of smaller ones: `git diff --stat main...HEAD | tail -1`.
...
- A one-shot terminal scan across a whole directory → `scripts/detect-smells.sh <dir>` (a reporter, always exits 0 — read every finding, false-positive profile documented per rule). Route it by the skill package's own directory, not the target project's cwd, since the agent's cwd at invocation time is the project being scanned: `bash <skill-dir>/scripts/detect-smells.sh <target-dir>`.
...
- [ ] Any smell fixed cites its `references/code-smells.md` entry and the detect command's result.
- [ ] Scope stayed within the stated task; anything spotted-but-out-of-scope was flagged, not fixed inline.
- [ ] `scripts/detect-smells.sh` ran on the touched directory at least once, findings reviewed rather than auto-applied.
