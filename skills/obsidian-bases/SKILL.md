---
name: obsidian-bases
description: Write, read, and optimize Obsidian Bases (`.base` files and `base` code blocks) across the full YAML schema — filters, formulas, properties, summaries, and views (`type`, `groupBy`, `sort`, `order`, `limit`, `columnSize`). Use when creating a base, embedding a base query, tuning `groupBy`/`sort` for a high-level project overview, debugging a base that returns no rows, adding a formula column, or converting Dataview into Bases. Not for Dataview/DataviewJS queries (different engine) or note prose formatting (use obsidian-markdown).
metadata:
  version: 1.0.0
---
